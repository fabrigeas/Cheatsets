...
let MyIndexedDB = class {
  ...
  insertOne(objectStoreName, data) {
    return this.getObjectStore(objectStoreName).then((objectStore) =>
...
function test() {
  const myIndexedDB = new MyIndexedDB("HelloIndexedDb");

  const objectStoreName = "Users";
  const objectStoreName2 = "users.duplicate";

  const keyStoreIndices = [
  ...
  const keyStoreKeyPath = "username";

  const manyUsers = [
    {
      name: "Snow",
      email: "feugang@gmail.com",
      username: "snow",
      gender: "male",
    },
    {
      name: "Sersey",
      email: "Sersey@gmail.com",
      username: "sersey",
      gender: "female",
    },
    {
  ...
  myIndexedDB
    .createObjectStore(objectStoreName, keyStoreIndices, keyStoreKeyPath)
    .catch((e) => console.log(e));
  myIndexedDB
    .createObjectStore(objectStoreName2, keyStoreIndices, keyStoreKeyPath)
    .catch((e) => console.log(e));
  myIndexedDB
    .createObjectStore("objectStoreName2", keyStoreIndices, keyStoreKeyPath)
    .catch((e) => console.log(e));

  myIndexedDB.insertOne(objectStoreName, {
    name: "Lanister",
    email: "John@gmnail.com",
    username: "lanister",
    gender: "male",
    other: "Code developper",
  });

  myIndexedDB.insertMany(objectStoreName, manyUsers);

  myIndexedDB
    .find(objectStoreName, "name", "Snow")
    .then((user) => console.log(user));

  myIndexedDB
    .findOne(objectStoreName, "snow")
    .then((user) => console.log(user));

  myIndexedDB
    .findbyIndex(objectStoreName, "name", "Sersey")
    .then((user) => console.log(user));

  myIndexedDB
    .remove(objectStoreName, "jimmy")
    .then((user) => console.log(user));

  myIndexedDB
    .update(objectStoreName, {
      name: "King of the throne",
      email: "jimmy@gmail.com",
      username: "jimmy",
      gender: "female",
    })
